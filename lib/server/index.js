...
app.get("*", function (req, res, next) {
	...
	var appHTML = "<!doctype html>\n\t" +
		...
		"tbody {font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif;} li { display:none}" +
		"body{background-color: #e7dbe7;}" +
		"#reg{background-color: #0c11a8; padding: 6px; text-decoration: none; border-radius: 6px; color: white; margin-top: 10px;}" +
		"#headerr{background-color:red}" +
		"#headline{background-color: #EEE ,padding: 10px}" +
		"#headline a{   background-color: #52bebe;padding: 6px;text-decoration: none;border-radius: 3px;color:white}" +
		"#headline a:empty {	display: none;}" +
		"#headline button {	background-color: red; padding: 6px; border: 1px solid red; border-radius: 3px;cursor:pointer}" +
		"#save{margin-top: 12px !important;background-color: #5d71ff; color: white; padding: 5px; border-radius: 4px;} " +
		"#regform{width: 200px; margin: auto; background-color: white; padding: 24px; border: gainsboro; border-radius: 10px; border: 1px solid; box-shadow: 4px 3px 5px grey;}" +
		"hr{display:none}" +
		"#accform{    box-shadow: 3px 3px 11px grey;width: 250px;margin: auto;	background-color: white;	padding: 26px;	border-radius: 23px;}" +
		"</style>" +
		"</head>" +
		"<body>" +
		"<div id=\"app\"></div>" +
